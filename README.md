...
插件只检查进程启动后新实时到达的群消息，并逐个读取消息中的 OneBot `text` segment；不会回放历史聊天，也不读取图片、语音、文件、卡片、引用正文或跨 segment 拼接内容。匹配前对文本和规则执行 Unicode NFKC、`casefold`，再移除空白字符与 Unicode `Cf` 格式字符；之后只做文字字面子串匹配，正则元字符没有特殊含义。重叠命中优先保留较长规则，一条消息命中多个不同规则时合并为一条纯文本告警。

告警只发送到实例私有配置的管理报告群。人工规则命中时可包含来源、发送者、时间、人工规则、受限长度摘录和消息编号；昵称、正文等不可信内容始终通过 OneBot 纯文本 segment 发送。后台受控目录按 `political_cn`、`sexual_explicit`、`gender_conflict`、`controversial_topics`、`anime_game_controversy`、`graphic_violence` 和 `terrorism` 七个稳定类别分片。除中国政治类外，受控规则命中可在管理群显示安全的分类中文名、命中词和受限摘录；`political_cn` 则由代码强制采用 `strict_hidden`，配置和导入文件均不能降低：报告以固定文案明确标为“政治敏感告警”，显示发送者群名片或昵称及 QQ，但不显示任何命中词、别名、目录分类、内部编号、数量、哈希、来源、文件路径或消息摘录。若昵称本身命中受保护规则，则只隐藏昵称并保留 QQ 作为身份主键。只要同一消息包含一个 `strict_hidden` 命中，人工规则和其他类别也全部隐藏，避免交叉泄漏。插件只负责提醒，不自动撤回消息、禁言、踢人、写入违规记录或改变任何业务判断。

超级管理员可在私聊，或在管理报告群明确 @ 当前机器人后执行 `/违禁词 列表`、`/违禁词 添加 <关键词>`、`/违禁词 删除 <编号>`；源群内不接受这些命令。这三个 QQ 命令只管理群内人工规则，永远不能列出、删除、探测或回显后台受控规则；即使人工规则列表为空而后台规则存在，`/违禁词 列表` 仍只报告人工列表为空。`/违禁词告警 开|关` 控制运行时开关，`/模块状态` 只显示功能状态，不显示任何后台规则元数据。

人工规则只保存在实例私有的 `data/content_alert/keywords.json`。大规模后台库位于 `data/content_alert/managed/`：`current.json` 原子指向一个不可变 generation，generation 的 manifest 记录分类说明、披露策略、数据版本、来源引用和每个 shard 的 SHA-256；词条记录 canonical term、别名、标签、置信度、状态、首次出现、复核时间和来源引用，方便以后接入 AI 复核或继续扩容。运行时平时复用已编译快照，仅在 pointer 改变时整代校验并切换；新代缺失、损坏、哈希不符、越界或包含符号链接时保留完整的 last-known-good，不会半加载。旧 `background_keywords.json` 保持不变，只在没有有效 managed generation 时作为全量 `strict_hidden` 回退。

后台导入使用 `scripts/import_content_alert_catalog.py --instance-root <实例绝对路径> --build <私有构建文件> --apply`。脚本先完整校验 active/stored 数量、Trie 节点和运行时文件大小，再以 `0700` 目录、`0600` 文件写入新 generation，备份旧 pointer 与 legacy 文件，最后在同目录原子替换 pointer；相同输入为无写入的幂等操作。`gender_conflict` 中只有带 `human-reviewed-gender-antagonism` 标签的人工复核词条才允许设为 `active`，宽泛语料应保留为 `shadow`，避免把普通辱骂或无关短语当作性别对立。执行 `--rollback` 只恢复上一 pointer（首代则移除 pointer 并在服务重启后回到未改动的 legacy 回退），不会删除 generation 或修改人工规则；若旧 generation 含未经人审的 active 性别对立项，回滚会拒绝切换并保持当前 pointer。备份以目标 generation 唯一命名；回滚并转入另一分支后若要再次激活同一个旧 generation，应使用新的目录 revision 重新构建，冲突时导入器会安全拒绝。构建文件、真实词条、运行数据、备份和导入输出均不得提交 Git，也不得复制到 QQ、日志或模型提示；公开扫描器会拒绝整个 `data/content_alert/` 路径。
