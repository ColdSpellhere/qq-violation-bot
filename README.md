...
`/穷鬼模式 开` 会把本实例面向用户的聊天和业务意图解析统一切换到独立配置的 `glm-4.7-flash`；切换是显式路由，GLM 调用失败不会偷偷回退到原付费模型。为避免免费模型配额被逐消息后台任务耗尽，模式开启期间暂停新的成员记忆提取、私聊事实/摘要和关系状态更新，并暂停领取已有的持久记忆任务；群聊和私聊原文仍照常保存，已有记忆仍可用于聊天，关闭模式后新后台任务恢复，但不会自动回溯补提取开启期间跳过的每一条消息。该模式同时停用群聊和私聊的新增图片理解、待处理图片恢复及视觉模型调用，并在组装聊天提示时剥离已有图片文字描述：纯图片消息（包括只 @ 机器人并引用图片）不触发 AI，图文消息只使用用户真实文字。已有图片描述不会被删除，关闭模式后仍可继续使用。违规证据图片的保存/查询和聊天表情包发送不属于图片理解，继续保持原行为。执行 `/穷鬼模式 关` 后恢复原文字模型、后台记忆整理和原有图片理解开关配置，无需重启。已经取得处理权的后台调用和图片任务可按切换前的供应商策略有界收尾；命令返回后新触发的任务按新模式执行。

GLM 的 `GLM_BASE_URL`、`GLM_API_KEY`、`GLM_MODEL` 和 `GLM_TIMEOUT` 只配置在各实例自己的私有 `.env` 中；真实密钥不得写入 Git、运行时状态或日志。配置不完整时拒绝开启穷鬼模式；若磁盘状态或环境默认已经要求开启、但重启时配置失效，实例会保留“开启”意图并失败关闭文字网关，不会静默改回付费模型。仅配置 GLM、没有原文字模型密钥的实例可以保持穷鬼模式运行，但 `/穷鬼模式 关` 会明确拒绝，避免先提示恢复成功、随后让全部文字调用失败；此时应先补回原模型配置。若要停用这个纯 GLM 实例的全部模型调用，可执行 `/模型网关 关`；对于同时配置原模型的普通实例，该命令只是退出统一网关并回到旧模型路径，不是通用 AI 总开关。新安装和旧运行时状态均默认关闭该模式；CArroT 与 kona 可分别配置和切换，互不共享状态或密钥。
...
联网搜索异常时先执行 `/联网搜索 关`，后续聊天立即回到不搜索路径，不需要回滚数据库。穷鬼模式异常时执行 `/穷鬼模式 关` 即恢复原模型、后台记忆整理和图片理解配置；`/模型网关 关` 会同时关闭穷鬼模式并切回旧模型路径，不能当作普通实例的 AI 总开关。多段回复不需要数据库迁移；代码回滚后已经写入的多条私聊助手消息仍是普通私聊原文。
