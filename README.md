...
人工规则只保存在实例私有的 `data/content_alert/keywords.json`。大规模后台库位于 `data/content_alert/managed/`：`current.json` 原子指向一个不可变 generation，generation 的 manifest 记录分类说明、披露策略、数据版本、锁定的来源 revision、来源文件 SHA-256、获取时间和每个 shard 的 SHA-256。公开敏感词仓库只能作为候选来源，不能整库直接上线；进入启用词库前必须完成适用范围、规范姓名、来源引用、重复/冲突和运行边界复核。`verification_status` 必须继续区分官方核验、研究资料候选和运维人工整理，不能把研究资料候选表述为官方数据。当前 `political_cn` 只有两个会产生告警的 subject：完整领导人姓名（`subject:leader_name`）和含义明确的敏感历史事件名称（`subject:historical_event`），两者均采用 `direct` 字面匹配；只要任一完整词条出现在单个 OneBot 文本段中就立即告警，不再要求额外政治语境。运行时仍可读取上一代 `same_segment_context`/`support_only` 目录，仅用于先恢复旧 pointer、再回退代码的紧急兼容；新构建的导入器会拒绝辅助语境词条，生产构建也不再生成它们。人工或官方核验条目必须带 `human-reviewed-political-scope`；批量研究候选可以改用 `source-screened-political-scope`，但仅表示其来源快照、级别筛选、规范化、出处和匹配约束已通过流水线校验，绝不表示逐人官方核验或历史名册绝对完整。`aliases` 必须为空。不得纳入领导人姓氏或姓名片段、事件简称不明确的短词、谐音、代称、拼音、拆字和自动扩展变体。生产启用词库不再提供可操作的 `shadow`/`active` 分流：所有入库敏感 subject 都必须以 `active` 常量写入并直接产生告警，导入器和运行时会拒绝启用类别中的 `shadow` 或 `disabled` 条目。候选材料应留在词库外的私有复核区，不能借 `shadow` 静默混入生产 generation。类别级 `enabled=false` 仅用于关闭兼容分类。运行时按规范姓名字符串匹配，因此同名人物必须在构建阶段合并为一个 `entity_ref` 姓名组；每位底层人物的来源记录、职级及冲突信息保留在实例私有身份旁表中，目录本身不伪装成逐实体模型。
