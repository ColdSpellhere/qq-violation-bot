...
`RANDOM_CHAT_ENABLED=true` 时，机器人以 `RANDOM_CHAT_PROBABILITY` 概率尝试参与目标群普通聊天。v1.0.2.3beta 默认概率为 5%。上下文会保留发送者 QQ、昵称、艾特对象和引用对象，避免把群友之间的话误当成对机器人说。成员记忆按群号和 QQ号存储，只接受成员本人明确表达且能回溯原消息的非敏感稳定特性；SQLite 为权威数据，便于人工查看的私有 JSON 镜像写入 `data/member_memory/`。运行时档案不会提交到 GitHub。
