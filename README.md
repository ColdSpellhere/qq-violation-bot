...
`/穷鬼模式 开` 会把本实例的全部文字模型任务统一切换到独立配置的 `glm-4.7-flash`，包括聊天、业务意图解析、成员记忆、私聊事实/摘要和关系状态；切换是显式路由，GLM 调用失败不会偷偷回退到原付费模型。该模式同时停用群聊和私聊的新增图片理解、待处理图片恢复及视觉模型调用，并在组装聊天提示时剥离已有图片文字描述：纯图片消息（包括只 @ 机器人并引用图片）不触发 AI，图文消息只使用用户真实文字。已有图片描述不会被删除，关闭模式后仍可继续使用。违规证据图片的保存/查询和聊天表情包发送不属于图片理解，继续保持原行为。执行 `/穷鬼模式 关` 后恢复原文字模型和原有图片理解开关配置，无需重启。已经取得处理权的图片任务可按切换前的供应商策略有界收尾，但其纯图片消息不会再进入 GLM；切换时尚未取得处理权的极少量群图片任务保持 `pending`，关闭模式后新图片立即恢复，旧 `pending` 会在下一次启动时按既有时间窗和数量上限恢复。命令返回后新触发的任务按新模式执行。
...
联网搜索异常时先执行 `/联网搜索 关`，后续聊天立即回到不搜索路径，不需要回滚数据库。穷鬼模式异常时执行 `/穷鬼模式 关` 即恢复原模型和图片理解配置；`/模型网关 关` 会同时关闭穷鬼模式并切回旧模型路径，不能当作普通实例的 AI 总开关。多段回复不需要数据库迁移；代码回滚后已经写入的多条私聊助手消息仍是普通私聊原文。
