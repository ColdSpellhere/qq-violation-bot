...
告警只发送到实例私有配置的管理报告群，包含来源、发送者、时间、命中规则、受限长度摘录和消息编号；昵称、正文等不可信内容始终通过 OneBot 纯文本 segment 发送。它只负责提醒，不自动撤回消息、禁言、踢人、写入违规记录或改变任何业务判断。超级管理员可在私聊，或在管理报告群明确 @ 当前机器人后执行 `/违禁词 列表`、`/违禁词 添加 <关键词>`、`/违禁词 删除 <编号>`；源群内不接受这些命令。`/违禁词告警 开|关` 控制运行时开关，`/模块状态` 显示当前状态。

规则保存在实例私有的 `data/content_alert/keywords.json`，只记录规则和更新元数据，不保存聊天内容；文件采用原子替换并支持热读取，不能提交到 Git。当前 MVP 告警通过 OneBot 直接发送，仅有进程内的重复消息保护，没有持久化 outbox、自动重试或历史补偿：QQ 离线、发送异常或进程中断时可能漏告警，失败只记录脱敏日志，不得回复成发送成功。后续正则、算法或 AI 分析应作为新的检测器接入 `content_alert` 边界，并继续保持业务处置隔离；本版仍只启用字面关键词检测。
