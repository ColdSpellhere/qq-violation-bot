...
统一模型网关（LLM Gateway）只负责模型传输，不拥有业务或聊天提示词。业务意图、聊天、成员记忆、私聊摘要/关系和图片描述仍由各自模块构造独立请求；聊天模型不能直接执行违规记录、禁言、状态或减数操作。网关为每个供应商复用持久异步连接池，并在供应商之上共享总并发和各调用域通道，统一管理 Base URL、API Key、模型、超时、重试和连接关闭，不复制密钥或为每次调用新建长期客户端。

`/模型切换 GLM` 会把本实例后续的群聊和私聊纯文字回复切换到独立配置的 `glm-4.7-flash`，`/模型切换 原模型` 可热切回原聊天文字模型，`/模型切换 状态` 用于查询当前选择，无需重启。`/聊天模型` 是同义命令；旧命令 `/穷鬼模式 开|关` 继续兼容，分别等价于切到 GLM 和切回原模型。切换只改变聊天文字模型，不改变聊天提示词、概率或路由：带当前原图的聊天继续使用原视觉模型，群聊和私聊图片下载、识别、恢复及已有图片描述保持原配置；业务意图解析、成员记忆、私聊事实/摘要、关系状态、原文归档、联网搜索、违规证据图片和表情包也都保持各自原有模型与开关。切换采用显式路由，GLM 调用失败不会暗中回退到原付费模型；已经开始的聊天请求按开始时选定的模型有界完成，命令返回后新开始的聊天文字请求使用新选择。

GLM 的 `GLM_BASE_URL`、`GLM_API_KEY`、`GLM_MODEL` 和 `GLM_TIMEOUT` 只配置在各实例自己的私有 `.env` 中；真实密钥不得写入 Git、运行时状态或日志。配置不完整时拒绝执行 `/模型切换 GLM`；若磁盘状态或环境默认已经选择 GLM、但重启时配置失效，实例会保留该选择并失败关闭聊天文字网关，不会静默改回付费模型。仅配置 GLM、没有原聊天文字模型密钥的实例可以继续使用 GLM，但 `/模型切换 原模型` 和兼容命令 `/穷鬼模式 关` 会明确拒绝，避免先提示切换成功、随后让聊天文字调用全部失败；此时应先补回原模型配置。若要停用这个纯 GLM 实例的全部模型调用，可执行 `/模型网关 关`；后续恢复时应执行 `/模型切换 GLM`，直接执行 `/模型网关 开` 会因缺少原模型而被拒绝，防止写入重启必然失败的状态。对于同时配置原模型的普通实例，`/模型网关 关` 只是关闭统一网关并把聊天文字模型恢复到原选择，不是通用 AI 总开关。新安装和旧运行时状态均默认使用原聊天文字模型；CArroT 与 kona 可分别配置和切换，互不共享状态或密钥。

网关错误只按类别记录和返回，例如配置、鉴权、超时、网络、限流、服务端、客户端、空内容和契约错误；日志不写请求正文、模型响应、Authorization、图片字节或 API Key。取消信号直接传播。网络失败、超时、普通供应商 HTTP 429 和可恢复的 5xx 只做有限退避重试；GLM 的 `1302/1305` 限流或繁忙响应在当前请求内快速失败，避免短间隔重试形成请求风暴。群内明确 @ 和白名单私聊遇到这类临时故障时发送固定繁忙提示，概率接话及非明确自动回复保持静默；鉴权、普通 4xx 和响应契约错误不盲目重试。

总并发和各调用域并发分别由 `LLM_GATEWAY_TOTAL_CONCURRENCY` 与 `LLM_GATEWAY_*_CONCURRENCY` 限制，并在原模型和 GLM 之间共享，切换重叠期也不会把限额翻倍。任务先取得自身通道，再进入总并发，避免排队中的聊天请求占住图片或业务容量。关闭服务时先停止接收新请求并有界等待在途调用；超时后取消剩余任务，再并行关闭两边连接。每次调用把任务类型、模型、输入/输出/总 token、延迟、状态、重试次数和脱敏错误类别写入 `llm_usage_events`；供应商未返回 token 时保持空值，当前不估算费用，`cost_microunits` 和 `cost_currency` 保持空值。统计写入失败不会改变业务或聊天结果。

Prompt Builder 只用于聊天，不用于业务意图。固定安全、方向和输出规则位于高权限 system 消息；`character.md`、近期上下文、成员事实、关系状态、未完话题、图片描述和当前消息均作为带标签的不可信数据放入 user 消息，即使其中含“忽略规则”等文本也不能覆盖权限和业务边界。`persona_data` 在固定安全边界内提供身份、性格和表达风格，但不能覆盖权限或说话者归属。每次请求会按精确 QQ号建立临时 `speaker_ref` 目录，并让历史消息、成员事实、@、引用作者和当前发送者使用同一套引用；system 消息还会把 `current_speaker_ref` 明确设为当前说话人身份锚点，禁止把其他成员的陈述、偏好或经历说成机器人自己或当前说话人的经历。第一人称只归属该条消息的作者，同名不同 QQ 不合并，引用作者也不会替代当前发送者。该目录不持久化、不跨请求共享，也不会把私聊记忆带进群聊。默认字符预算如下：人设 2000、最近上下文最多 20 条/6000、成员事实 1200、关系状态 600、未完话题最多 5 条/400、图片描述 2000、当前消息 2000、最终请求 12000。超限时按确定顺序裁剪最旧上下文等可裁剪数据，同时同步裁剪无用说话者目录项；固定规则和当前消息不会被静默丢弃，预算按最终转义后的实际内容计算。

聊天联网搜索使用 Tavily，默认关闭。它只会在白名单私聊或群内明确 @ 机器人时，针对“搜一下/查一下”等明确请求或明显时效问题查询；普通随机群聊、业务意图、违规判断、禁言、状态和减数策略永远不会调用搜索。查询只取当前消息并限制为 200 字，不发送 QQ号、昵称、历史、记忆、关系状态或图片；最多读取 5 条结果、4000 字，结果作为 `<web_search_data>` 不可信 user 数据注入。搜索失败会降级为普通聊天，并约束模型不得假装已经查到实时信息。每个实例必须在自己的 `.env` 中配置 `TAVILY_API_KEY`，不得写入 Git、运行时状态或日志；基础查询会消耗 Tavily 账户额度。联网搜索是独立开关，聊天文字模型切换不会替你自动关闭；如需停止 Tavily 配额消耗，应另行执行 `/联网搜索 关`。
...
异常时先关闭最窄子开关。例如视觉异常用 `/模型网关 视觉 关`，聊天异常先 `/提示构建 关`，仍异常再 `/模型网关 聊天 关`；业务异常立即 `/模型网关 业务 关`，不要关闭整个业务模块。`/模型网关 关` 会退出统一网关并切回旧模型调用路径，而不是普通实例的 AI 总开关；只有未配置原文字模型的纯 GLM 实例会因此停止模型调用。这些止损不删除使用统计、记忆或关系数据，也不需要数据库回滚。只有 schema 迁移损坏时才按前述停服、保留故障库、恢复在线备份和再次 `quick_check` 的流程处理。
...
CONTENT_ALERT_REPORT_GROUP_ID=0
```
...
紧急止损时，优先使用对应的 QQ 命令，无需重启或回滚代码：`/业务 关` 停止新业务请求、v1.0.2beta 与旧版自动维护提醒，以及尚未开始的周报生成、通知和上传；每次实际外发前都会再次检查开关。`/聊天 关` 停止全部聊天入口；`/群聊 关` 只停群聊、归档和成员记忆；`/私聊 关` 只停私聊；`/私聊记忆 关`、`/关系状态 关` 和 `/记忆治理 关` 分别停止新的持久记忆、关系更新和治理入口；`/违禁词告警 关` 立即阻止后续关键词检测和告警外发。开关修改失败会保留旧状态并返回失败信息，不能把失败当作已生效。

联网搜索异常时先执行 `/联网搜索 关`，后续聊天立即回到不搜索路径，不需要回滚数据库。GLM 聊天异常时执行 `/模型切换 原模型` 即可恢复原聊天文字模型；兼容命令 `/穷鬼模式 关` 效果相同。切换不会启停图片、业务或记忆功能。`/模型网关 关` 会同时把聊天文字模型恢复为原模型并关闭统一网关，不能当作普通实例的 AI 总开关。多段回复不需要数据库迁移；代码回滚后已经写入的多条私聊助手消息仍是普通私聊原文。
...
`plugins.content_alert` 是独立、默认关闭的实时文字告警插件，不调用 AI 或 LLM。只有静态配置完整且 `CONTENT_ALERT_ENABLED=true` 的实例才注册处理器；`CONTENT_ALERT_SOURCE_GROUP_IDS` 中的全部源群都必须显式属于 `MONITOR_ONLY_GROUP_IDS`，并且不能与 `CONTENT_ALERT_REPORT_GROUP_ID` 或业务 `TARGET_GROUP_ID` 相同。真实源群、报告群和规则只存在于各实例私有配置与数据目录；未配置实例不会注册告警 matcher，也不会创建规则文件。源群继续遵守 monitor-only 硬隔离，不进入聊天、归档、图片理解、成员记忆或业务流程。

插件只检查进程启动后新实时到达的群消息，并逐个读取消息中的 OneBot `text` segment；不会回放历史聊天，也不读取图片、语音、文件、卡片、引用正文或跨 segment 拼接内容。匹配前对文本和规则执行两轮 Unicode NFKC 与 `casefold`，并移除空白、控制/格式/代理字符和 Unicode Default-Ignorable 范围内可用于视觉绕过的字符；私用区和普通未分配码点仍保留并中断字面匹配。之后只做文字字面子串匹配，正则元字符没有特殊含义。重叠命中优先保留较长规则，一条消息命中多个不同规则时合并为一条纯文本告警。受控扫描同时限制单个 text segment 和整条消息的 segment 数、原文长度、候选/命中数及姓名—语境比较次数；目录扫描和可见摘录定位均在线程中执行，并由实例级小并发信号量约束。超限时不静默漏过，而是向管理群发送不含正文和词条的单条“关键词扫描保护告警”。

告警只发送到实例私有配置的管理报告群。人工规则命中时可包含来源、发送者、时间、人工规则、受限长度摘录和消息编号；昵称、正文等不可信内容始终通过 OneBot 纯文本 segment 发送。后台受控目录仍按 `political_cn`、`sexual_explicit`、`gender_conflict`、`controversial_topics`、`anime_game_controversy`、`graphic_violence` 和 `terrorism` 七个稳定类别分片，以保持构建格式兼容。收窄后的服务器私有生产构建只启用 `political_cn` 的 active 词条，其他类别保持 disabled；政治类披露策略固定为 `management_visible`，报告以“政治敏感告警”标识，显示发送者、完整命中词和受限长度摘录。插件只负责提醒，不自动撤回消息、禁言、踢人、写入违规记录或改变任何业务判断。

超级管理员可在私聊，或在管理报告群明确 @ 当前机器人后执行 `/违禁词 列表`、`/违禁词 添加 <关键词>`、`/违禁词 删除 <编号>`；源群内不接受这些命令。这三个 QQ 命令只管理群内人工规则，永远不能列出、删除、探测或回显后台受控规则；即使人工规则列表为空而后台规则存在，`/违禁词 列表` 仍只报告人工列表为空。`/违禁词告警 开|关` 控制运行时开关，`/模块状态` 只显示功能状态，不显示任何后台规则元数据。关闭开关会在扫描后和最终串行发送边界再次拦截排队事件；已经进入 OneBot 远端调用的单次请求无法事务性撤回，属于不可撤销的在途请求边界。

人工规则只保存在实例私有的 `data/content_alert/keywords.json`。大规模后台库位于 `data/content_alert/managed/`：`current.json` 原子指向一个不可变 generation，generation 的 manifest 记录分类说明、披露策略、数据版本、锁定的来源 revision、来源文件 SHA-256、获取时间和每个 shard 的 SHA-256。公开敏感词仓库只能作为候选来源，不能整库直接上线；进入启用词库前必须完成适用范围、规范姓名、来源引用、重复/冲突和运行边界复核。`verification_status` 必须继续区分官方核验、研究资料候选和运维人工整理，不能把研究资料候选表述为官方数据。当前 `political_cn` 只有两个会产生告警的 subject：完整领导人姓名（`subject:leader_name`）和含义明确的敏感历史事件名称（`subject:historical_event`）；另允许少量高特异度政治语境（`subject:political_context`）作为 `support_only` 辅助项，它们不是敏感词，单独出现永不告警。历史事件采用 `direct`；领导姓名必须与 strong 语境位于同一 text segment，规范化间距不超过 12 个字符，weak 语境不能触发。人工或官方核验条目必须带 `human-reviewed-political-scope`；批量研究候选可以改用 `source-screened-political-scope`，但仅表示其来源快照、级别筛选、规范化、出处和匹配约束已通过流水线校验，绝不表示逐人官方核验或历史名册绝对完整。`aliases` 必须为空。不得纳入领导人姓氏或姓名片段、事件简称不明确的短词、谐音、代称、拼音、拆字和自动扩展变体。v2 的生产启用词库不再区分 `shadow` 和 `active`：所有敏感 subject 一旦进库都必须为 `active` 并产生告警，导入器和运行时会拒绝启用类别中的 `shadow` 或 `disabled` 条目；`support_only` 仅是非敏感的组合检测辅助语境。候选材料应留在词库外的私有复核区，不能借 `shadow` 静默混入生产 generation。类别级 `enabled=false` 仅用于关闭兼容分类。运行时按规范姓名字符串匹配，因此同名人物必须在构建阶段合并为一个 `entity_ref` 姓名组；每位底层人物的来源记录、职级及冲突信息保留在实例私有身份旁表中，目录本身不伪装成逐实体模型。

词条记录 canonical term、标签、置信度、状态、首次出现、复核时间和来源引用，方便后续人工复核或扩容。运行时平时复用已编译快照，仅在 pointer 改变时整代校验并切换；新代缺失、损坏、哈希不符、越界或包含符号链接时保留完整的 last-known-good，不会半加载。旧 `background_keywords.json` 保持不变且默认不合并；`--include-legacy-background` 只保留给显式允许的 v1 兼容迁移，v2 会在写入前拒绝这种 shadow 合并。待复核旧词不能自动补写、伪造政治类复核标签或进入 v2 启用 generation。当没有任何有效 managed generation 时，运行时仍可把 legacy 文件作为全量 `strict_hidden` 故障回退。

后台导入使用 `scripts/import_content_alert_catalog.py --instance-root <实例绝对路径> --build <服务器私有构建文件> --apply`；只有经过单独审查且显式允许的 v1 兼容迁移才可附加 `--include-legacy-background`。脚本先完整校验 active/stored 数量、v2 启用类别无静默条目、政治类范围筛选或人工复核标签、复核时间、互斥 subject 标签、空 aliases、被引用来源的 revision 与 SHA-256、Trie 节点和运行时文件大小，再以 `0700` 目录、`0600` 文件写入新 generation，备份旧 pointer 与 legacy 文件，最后在同目录原子替换 pointer；相同输入为无写入的幂等操作。执行 `--rollback` 只恢复上一 pointer（首代则移除 pointer 并在服务重启后回到未改动的 legacy 回退），不会删除 generation 或修改人工规则；升级和回滚校验兼容旧 `strict_hidden` 政治 generation，但所有新构建仍必须采用 `management_visible`。备份以目标 generation 唯一命名；回滚并转入另一分支后若要再次激活同一个旧 generation，应使用新的目录 revision 重新构建，冲突时导入器会安全拒绝。完整私有构建、批量词条清单、运行数据、备份和导入明细均不得提交 Git，也不得复制到 QQ、日志或模型提示；`management_visible` 只允许把当前消息实际命中的单个词条和受限摘录发送到实例私有管理群。公开扫描器会拒绝整个 `data/content_alert/` 路径。Git 中只保留结构、验证逻辑、合成测试夹具和不含真实词条的文档。

当前 MVP 告警通过 OneBot 直接发送，仅有进程内的重复消息保护，没有持久化 outbox、自动重试或历史补偿：QQ 离线、发送异常或进程中断时可能漏告警，失败只记录脱敏日志，不得回复成发送成功。后续正则、算法或 AI 分析应作为新的检测器接入 `content_alert` 边界，并继续保持业务处置隔离；本版仍只启用字面关键词检测。
