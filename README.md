...
告警只发送到实例私有配置的管理报告群。人工规则命中时可包含来源、发送者、时间、人工规则、受限长度摘录和消息编号；昵称、正文等不可信内容始终通过 OneBot 纯文本 segment 发送。后台受控规则命中时采用强制防泄漏格式：不显示后台词、规则编号、数量、哈希或文件路径，同时隐藏整段消息摘录和发送者昵称，只保留来源群、发送者 QQ、时间、消息编号及“受保护规则命中”的固定提示；人工与后台规则混合命中时也执行同样的正文和昵称隐藏，仅允许展示人工规则详情。插件只负责提醒，不自动撤回消息、禁言、踢人、写入违规记录或改变任何业务判断。
...
人工规则保存在实例私有的 `data/content_alert/keywords.json`，后台受控规则保存在物理隔离的 `data/content_alert/background_keywords.json`；两者只记录规则和更新元数据，不保存聊天内容。QQ 命令对象只持有人工规则存储，检测服务则独立读取两套存储，避免相同规则编号跨文件碰撞或把后台存储误接入管理命令。文件采用原子替换并支持热读取，目录与其中的正式、备份和临时规则文件均不得提交到 Git；公开仓库扫描器会显式拒绝整个 `data/content_alert/` 路径，即使有人绕过 `.gitignore` 强制跟踪也会失败。
