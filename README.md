...
质询/最后警告：需要二次确认。质询默认结果为 `通过`，状态变为 `已质询`；最后警告状态变为 `最后警告`。如果结果标明 `已移出` 或 `已拉黑`，会进入锁定状态。
...
后台任务每分钟检查：最近 7 天无新增有效违规记录且当前次数大于 0，则 `deduct_count +1` 并写日志、群内通知；最后警告超过 90 天无新增有效违规记录，自动恢复为 `已质询` 并通知。
...
每周日 00:10 生成 XLSX 周报，包含本周操作日志和当前各分区成员统计。生成后会尝试上传到群文件，失败时返回服务器路径。
