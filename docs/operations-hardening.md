...
CI 强制检查当前树和本次变更新增的每个提交：push 使用事件的 before SHA，PR 使用 base SHA；不接受缺失或不可解析的基线。首次分支推送无祖先时检查全历史。人工 `--history` 仍审查全部可达历史且保持原拒绝规则。2026-09-05 全历史审查发现既有公开文档曾写入真实机器人身份，当前树已经不含该值；没有为该值增加例外，也没有改写已发布历史。新增提交检查通过只证明本次公开边界，不代表旧历史已清理。
